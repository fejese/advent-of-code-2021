...
class Game:
    ...
    def print_no_parent(self) -> None:
        print(f"Game(corridor={self.corridor}, rooms={self.rooms}, cost={self.cost})")
...
def read_input() -> Game:
    with open(INPUT_FILE_NAME, "r") as input_file:
        ...
        rooms: List[List[str]] = [
            "".join(lines[y][x] for y in [2, 3, 4, 5]) for x in [3, 5, 7, 9]
        ]
    return Game(corridor="." * 11, rooms=rooms)
    ...
    while games_to_process:
        ...
        for room_no, letter in enumerate("ABCD"):
            ...
            if letter in corr and game.rooms[room_no].strip(f".{letter}") == "":
                ...
                letters_already_in_room: int = game.rooms[room_no].count(letter)
                space_in_room: int = 4 - letters_already_in_room
                new_room: str = (
                    "." * (space_in_room - 1) + letter * (letters_already_in_room + 1)
                )
                room_steps: int = space_in_room
                new_rooms: List[str] = [
            ...
            if game.rooms[room_no].strip(f".{letter}") != "":
                space_in_room: int = game.rooms[room_no].count(".")
                new_room: str = (
                    "." * (space_in_room + 1) + game.rooms[room_no][space_in_room + 1 :]
                )
                room_steps: int = space_in_room + 1
                found_letter: str = game.rooms[room_no][space_in_room]
                new_rooms: List[str] = [
                    new_room if i == room_no else room
                    for i, room in enumerate(game.rooms)
                ]
