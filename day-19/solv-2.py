...
scanner_poss: List[P] = [(0, 0, 0) for _ in range(len(scans))]
                    ...
                    for o_left in left:
                        for o_right in rotated_right:
                            ...
                            if len(intersection) >= 12:
                                ...
                                print(f"  Position: {o_diff}")
                                scanner_poss[ridx] = o_diff
                                raise FoundOne()
...
max_distance: int = 0
for lidx, left in enumerate(scanner_poss):
    for right in scanner_poss[lidx + 1 :]:
        distance: int = sum(abs(rc - lc) for lc, rc in zip(left, right))
